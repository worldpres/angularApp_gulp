...
var htmlMin = require('gulp-htmlmin'); //optymalizacja html
...
gulp.task('sass', function(){
	return gulp.src(cfg.scssin)	//odczytanie plików scss z katalogu scss i nizej
		...
		.pipe(sass().on('error', sass.logError)) //uruchomienie sassa na plikach z src/scss
		.pipe(autoprefixer({
...
gulp.task('serve', ['sass'], function() { //zadnie serve które też uruchamia zadanie sass
	...
	gulp.watch([cfg.htmlin, cfg.jsin, cfg.phpin], ['reload']); //sledzenie zmian w plikach html, jesli zmiana to przeladuje strone
